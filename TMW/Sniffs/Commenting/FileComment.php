...
class TMW_Sniffs_Commenting_FileComment implements PHP_CodeSniffer_Sniff
{
    ...
    protected function processPackage(PHP_CodeSniffer_File $phpcsFile, array $tags)
    {
        ...
        foreach ($tags as $tag) {
			...
			// just nakes sure the package name starts with a letter and is followed by letters and numbers
			// and prevents the weird magic of forcing capitalisation on the name that seems to be happening in PHP_CodeSniffer::isUnderscoreName()
			if(preg_match('/^\p{L}[\p{L}\p{N}]+$/', $content) )
				continue;
